use std::sync::{atomic::{AtomicBool, Ordering as MemOrdering}, Mutex};

use rodio::{Sample, Source};
use cpal::FromSample;

use crate::{data::TimeSpec, prelude::*};

pub(super) type MySample = f32;
type BoxedSource = Box<dyn Source<Item = MySample> + Send + Sync>;

/// Audio Source that mixes many sounds with timings aligned to game ticks
///
...
impl PrecisionMixerController {
    ...
    fn play_at_sample_number<T, S>(&mut self, start_at_sample_number: Option<u64>, source: T, volume: f32, pan: f32)
    where
    ...
    pub fn play_immediately<T, S>(&mut self, source: T, volume: f32, pan: f32)
    where
        T: Source<Item = S> + Send + Sync + 'static,
        S: Sample + Send + 'static,
        MySample: FromSample<S>,
    ...
    pub fn play_at_time<T, S>(&mut self, time: TimeSpec, source: T, volume: f32, pan: f32)
    where
        T: Source<Item = S> + Send + Sync + 'static,
        S: Sample + Send + 'static,
        MySample: FromSample<S>,
    {
        let dur = Duration::from(time);
    ...
    pub fn play_at_tick<T, S>(&mut self, tick: u32, offset_nanos: i32, source: T, volume: f32, pan: f32)
    where
        T: Source<Item = S> + Send + Sync + 'static,
        S: Sample + Send + 'static,
        MySample: FromSample<S>,
    ...
    pub fn trigger_reset(&self, reset: bool) {
        self.reset_triggered.store(reset, MemOrdering::SeqCst);
    }
}
...
impl Iterator for PrecisionMixer {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.controller.reset_triggered.load(MemOrdering::SeqCst) {
            self.sample_count = 0;
        }
