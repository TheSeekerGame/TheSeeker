...
use crate::physics::{Collider, SpriteShapeMap};
use crate::prelude::*;
...
impl<S: States> Plugin for AssetsPlugin<S> {
    fn build(&self, app: &mut App) {
        ...
        app.add_plugins((
            TomlAssetPlugin::<crate::ai::asset::EnemyArchetype>::new(&[
                "arch.toml",
            ]),
        ...
        app.add_systems(
            Update,
            hot_reload_archetype_fsms
                .run_if(on_event::<AssetEvent<crate::ai::EnemyArchetype>>),
        );
    }
}
...
fn merge_archetype_overrides(
    ...
    for (id, mut archetype) in to_merge {
        if let Some(base_name) = &archetype.base {
            ...
            if let Some(base_handle) = preloaded
                .get_single_asset::<crate::ai::EnemyArchetype>(&base_key)
            {
                if let Some(base_arch) = arch_assets.get(&base_handle) {
                    ...
                    error!(
                        "Base archetype {} not found for override",
                        base_name
                    );
                }
            } else {
                error!(
                    "Base archetype {} not in preloaded assets",
                    base_name
                );
            }
        }
...
fn compile_ai_fsm_assets(
    ...
    for (handle, archetype) in arch_assets.iter() {
        if let Err(e) = archetype.validate() {
            error!(
                "Archetype validation failed for {:?}: {}",
                handle, e
            );
        }
    }
...
fn populate_collider_map(
    ...
    for (h_image, h_layout) in iter_assets {
        ...
        let mut image = match image_origin
        ...
        for (i, anim_frame_rect) in layout.textures.iter().enumerate() {
            ...
            // Flip X
            let verts_flipped_x: Vec<Vec2> =
                verts.iter().map(|p| Vec2::new(-p.x, p.y)).collect();
            ...
            // Flip both X and Y
            let verts_flipped_xy: Vec<Vec2> =
                verts.iter().map(|p| Vec2::new(-p.x, -p.y)).collect();
            let shape_flipped_xy = match Collider::convex_hull(
                &verts_flipped_xy,
            ) {
                Some(collider) => collider.raw,
                None => {
            ...
            AssetEvent::Modified { id } => {
                ...
                if let Some(archetype) = arch_assets.get(*id).cloned() {
                    ...
                    if let Some(base_name) = &archetype.base {
                        ...
                        let base_key = format!("arch.{}", base_name);
                        if let Some(base_handle) = preloaded
                            .get_single_asset::<crate::ai::EnemyArchetype>(
                            &base_key,
                        ) {
                            if let Some(base_arch) =
                                arch_assets.get(&base_handle)
                            {
                    ...
                    if archetype.base.is_none() {
                        ...
                        for (override_handle, override_arch) in
                            arch_assets.iter()
                        {
                    ...
                    for (key, new_handle) in compiled_map {
                        if let Some(old_handle) = preloaded
                            .get_single_asset::<crate::ai::CompiledFsm>(
                            &key,
                        ) {
                            if old_handle != new_handle {
                                // Clone the new FSM data first to avoid borrowing conflicts
                                if let Some(new_fsm) =
                                    compiled_assets.get(&new_handle)
                                {
                                    let new_fsm_data = new_fsm.clone();
                                    if let Some(old_fsm) =
                                        compiled_assets.get_mut(&old_handle)
                                    {
                    ...
                    for mut fsm in fsm_instances.iter_mut() {
                        ...
                        {
                            let expected_len =
                                compiled.inner.cooldown_names.len();
