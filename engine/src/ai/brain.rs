...
//!
//! Compilation flow:
//! 1. Parse TOML FSM and resolve state names to dense `StateId`s
//! 2. Expand `$CONSTANT` references using the archetype stats
//! 3. Build compact action/condition tables (slots and cooldown indices)
//! 4. Produce an immutable `CompiledFsm` shared by all instances

use super::asset::{
    ArchetypeStats, EnemyFsm, FsmAction, FsmCondition, FsmState, FsmTransition,
};
use bevy::prelude::*;
use std::borrow::Cow;
...
pub struct CompiledFsmInner {
    ...
    /// ID to state name mappings (debugging/inspector only)
...
pub enum CompiledAction {
    ...
    /// Spawn attack prefab
    SpawnAttack { key: String, dmg: f32 },

    /// Set cooldown timer
    Cooldown { name: String, ticks: u16 },
    ...
    pub fn collect_animation_keys(&self) -> Vec<String> {
        ...
        let extract_keys = |action: &CompiledAction| -> Option<String> {
            match action {
                ...
                CompiledAction::Delayed { inner, .. }
                | CompiledAction::StateDelayed { inner, .. } => {
                    match inner.as_ref() {
                        CompiledAction::PlayAnim(key) => Some(key.to_string()),
                        _ => None,
                    }
        ...
        for state_actions in self
            .inner
            .logic_state_actions
            .iter()
            .chain(&self.inner.movement_state_actions)
        {
            animation_keys
                .extend(state_actions.on_enter.iter().filter_map(extract_keys));
            animation_keys
                .extend(state_actions.tick.iter().filter_map(extract_keys));
        }
        ...
        for rule in self
            ...
            .logic_rules
            .iter()
            .chain(&self.inner.movement_rules)
        {
...
pub fn compile_fsm(
    ...
    for state in &fsm.state {
        match state.track.as_str() {
            ...
            other => {
                return Err(format!(
                    "Unknown track '{}' in state definition '{}'",
                    other, state.name
                ));
            },
        }
    ...
    let mut logic_state_actions =
        vec![StateActions::empty(); logic_state_names.len()];
    for (name, actions) in logic_actions_temp {
        let idx = logic_states[&name] as usize;
        logic_state_actions[idx] = actions;
    }

    let mut movement_state_actions =
        vec![StateActions::empty(); movement_state_names.len()];
    ...
    let start_logic = *logic_states.get(&fsm.start.logic).ok_or_else(|| {
        format!(
            "Start logic state '{}' not found",
            fsm.start.logic
        )
    })?;
    let start_movement =
        *movement_states.get(&fsm.start.movement).ok_or_else(|| {
            format!(
                "Start movement state '{}' not found",
                fsm.start.movement
            )
        })?;
    ...
    for transition in &fsm.transition {
        let target_vec = match transition.track.as_str() {
            ...
            other => {
                return Err(format!(
                    "Unknown track '{}' in transition rule",
                    other
                ))
            },
        };
    ...
    // ---------------------------------------------------------------------
    // Add an implicit fallback rule so at least one rule matches each tick,
    // preventing a state vacuum. This no-op rule matches `Any`, has `Always`
    // as its sole condition, performs no actions, and does not transition.
    // Priority is `u16::MAX` so authored rules always take precedence.
    // ---------------------------------------------------------------------
...
fn compile_transition(
    ...
    // Parse from pattern
    let from_pattern = parse_from_pattern(
        &transition.from,
        logic_states,
        movement_states,
    )?;
    ...
    let target_state =
        state_map.get(&transition.to).copied().ok_or_else(|| {
            format!(
...
fn parse_from_pattern(
    ...
    if from.contains('|') {
        ...
        for state_name in from.split('|') {
            ...
            if let Some(&id) =
                logic_states.get(name).or_else(|| movement_states.get(name))
            {
                state_ids.push(id);
            } else {
                return Err(format!(
                    "State '{}' not found in from pattern",
                    name
                ));
            }
        }
        ...
        // Single state
        if let Some(&id) =
            logic_states.get(from).or_else(|| movement_states.get(from))
        {
    ...
    if let Some(n) = value.as_f64() {
        ...
        if s.starts_with('$') {
            ...
            if let Some(stats) = archetype_stats {
                ...
                Err(format!(
                    "No archetype stats available for constant: {}",
                    const_name
                ))
            }
        } else {
...
fn compile_conditions(
    ...
    for condition in conditions {
        let compiled = compile_single_condition(
            condition,
            archetype_stats,
            slot_names,
            cooldown_names,
...
fn compile_single_condition(
    ...
    match condition {
        FsmCondition::Simple(name) => match name.as_str() {
        ...
        FsmCondition::WithParam { params } => {
            ...
            if let Some(distance_lt) = params.get("distance_lt") {
                let distance =
                    parse_distance_value(distance_lt, archetype_stats)?;
                return Ok(CompiledCondition::DistanceLt(distance));
            }

            if let Some(distance_gt) = params.get("distance_gt") {
                let distance =
                    parse_distance_value(distance_gt, archetype_stats)?;
                return Ok(CompiledCondition::DistanceGt(distance));
            }

            // Timer condition
            if let Some(timer_gt) =
                params.get("TimerGt").and_then(|v| v.as_u64())
            {
                return Ok(CompiledCondition::TimerGt(
                    timer_gt as u16,
                ));
            }
            ...
            if let Some(expected) =
                params.get("IsGrounded").and_then(|v| v.as_bool())
            {
            ...
            // CooldownReady
            if let Some(name) =
                params.get("CooldownReady").and_then(|v| v.as_str())
            {
            ...
            Err(format!(
                "Unknown parameterized condition: {:?}",
                params
            ))
        },
    }
...
fn compile_action(
    ...
    match action {
        FsmAction::Simple(name) => match name.as_str() {
            "face_player" => Ok(CompiledAction::FacePlayer),
            _ => Err(format!(
                "Unknown simple action: {}",
                name
            )),
        },
        FsmAction::WithParam { params } => {
            // Handle delayed actions first
            if let Some(tick_val) =
                params.get("at_tick").or_else(|| params.get("at_anim_tick"))
            {
                ...
                if inner_params.is_empty() {
                    return Err(
                        "at_anim_tick provided but no inner action specified"
                            .to_string(),
                    );
                }

                let inner = compile_action(
                    &FsmAction::WithParam {
                        params: inner_params,
                    },
                    archetype,
                ...
                return Ok(CompiledAction::Delayed {
                    tick,
                    inner: Box::new(inner),
                });
            }
            ...
            if let Some(tick_val) = params.get("at_state_tick") {
                ...
                if inner_params.is_empty() {
                    return Err(
                        "at_state_tick provided but no inner action specified"
                            .to_string(),
                    );
                }
                ...
                return Ok(CompiledAction::StateDelayed {
                    tick,
                    inner: Box::new(inner),
                });
            }
            ...
            if let Some(anim) = params.get("play_anim").and_then(|v| v.as_str())
            {
                let full_key = if anim.starts_with("anim.") {
                    ...
                    arch.anim.get(anim).cloned().ok_or_else(|| {
                        format!(
                            "Animation key '{}' missing in archetype {}",
                            anim, arch.id
                        )
                    })?
                };
                return Ok(CompiledAction::PlayAnim(Cow::Owned(
                    full_key,
                )));
            }
            ...
            if let Some(vel) = params.get("set_vel").and_then(|v| v.as_str()) {
                ...
                if parts.len() == 2 {
                    let x = parts[0].trim().parse::<f32>().map_err(|_| {
                        format!("Invalid velocity X: {}", parts[0])
                    })?;
                    let y = parts[1].trim().parse::<f32>().map_err(|_| {
                        format!("Invalid velocity Y: {}", parts[1])
                    })?;
            ...
            if let Some(speed_val) = params.get("set_vel_towards_player") {
                ...
                return Ok(CompiledAction::SetVelTowardsPlayer(
                    speed,
                ));
            }
            ...
            if let Some(attack_params) =
                params.get("spawn_attack").and_then(|v| v.as_object())
            {
                let key = attack_params
                    .get("key")
                    .and_then(|v| v.as_str())
                    .ok_or("spawn_attack missing key")?
                    .to_string();

                let dmg_val = attack_params
                    .get("dmg")
            ...
            if let Some(cd_params) =
                params.get("Cooldown").and_then(|v| v.as_object())
            {
                let name = cd_params
                    .get("name")
                    .and_then(|v| v.as_str())
                    .ok_or("Cooldown action missing name")?;
                let ticks_val = cd_params
                    .get("ticks")
                ...
                return Ok(CompiledAction::Cooldown {
                    name: name.to_string(),
                    ticks,
                });
            }
...
fn compile_state_actions(
    ...
    // Process on_enter actions. Any Delayed/StateDelayed wrapper (even with tick == 0)
    // is routed to the per-frame `tick` list so it can re-fire on animation loops per spec.
    // All other actions execute once on state entry.
    for a in &state.on_enter {
        let compiled = compile_action(
            a,
            archetype,
            archetype_stats,
            cooldown_names,
        ...
        match compiled {
            CompiledAction::Delayed { .. }
            | CompiledAction::StateDelayed { .. } => {
    ...
    for a in &state.tick {
        tick_compiled.push(compile_action(
            a,
            archetype,
            archetype_stats,
            cooldown_names,
